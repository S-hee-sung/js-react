...
function App() {
  ...
  // props로 전달해야 할 함수를 만들 때는 useCallback()를 사용해본다!
  ...
  const handleRemove = useCallback((id) => {
    ...
    // 방법2 - 배열의 내장 함수 이용
    // filter(): 기존의 배열은 변경하지 않고 특정 조건을 만족하는 요소들만 따로 추출하여
    // 새로운 배열을 만듦
    // 테스트 함수에서는 true 또는 false를 반환해야 하며, 여기서 true를 반환하는 경우만 새로운
    // 배열에 포함됨
  ...
  // todose 배열의 특정 요소를 수정하기 위한 handleToggle() 함수 정의
  ...
  const handleToggle = useCallback((id) => {
    ...
    // 로컬 스토리지에 저장
    localStorage.setItem('todos', JSON.stringify(setTodos(todos.map((todo) => 
    todo.id === id ? { ...todo, checked: !todo.checked } : todo
  ))));
  },[todos]);
