...
function ReservationRefactoring() {
  ...
  const handleInputChange = (e) => { 
    ...
    // setInputs(inputs);
    // 이런식으로 직접 수정하면 안됨(inputs가 가르키는 객체의 내부 데이터만 바뀐것이지 
    // 참조값은 변하지 않음)
    // 그 대신 새로운 객체를 만들어서 새로운 객체에 변화를 주고 이를 상태로 사용해야 함
    // 이런 작업을 "불변성을 지킨다" 라고 부름
    // 불변성을 지켜주어야만 React 컴포넌트에서 상태가 업데이트 됐음을 감지 할 수 있고 재랜더링이 진행됨
  ...
  return (  
    <>
      ...
      {/* <br /> */}
      <button type="button" onClick={handleSubmit} style={{ padding:10, backgroundColor: 'aqua' }}>
        제-출
      </button>
    </>
  );
